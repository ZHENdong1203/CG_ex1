...
void myInit()
{
	...
	//ADD TEXTURE
	makeCheckImage();
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);//不使用对齐

#ifdef GL_VERSION_1_1
	glGenTextures(1, &texName);//创建纹理对象，指向纹理索引
	glBindTexture(GL_TEXTURE_2D, texName);//绑定纹理
#endif
	//确定纹理如何应用到每个像素上，纹理过滤函数
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);//S方向贴图
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);//T方向贴图
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);//线性过滤
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);//线性插值
#ifdef GL_VERSION_1_1
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, checkImageWidth, checkImageHeight,
		0, GL_RGBA, GL_UNSIGNED_BYTE, checkImage);//为纹理对象指定纹理
#else
...
void myGlutDisplay() //绘图函数， 操作系统在必要时刻就会对窗体进行重新绘制操作
{
	...
	glRotatef(rotationX, 1.0, 0.0, 0.0);//沿方向向量（1,0,0）旋转rotationX度
	glRotatef(rotationY, 0.0, 1.0, 0.0);////沿方向向量（0,1,0）旋转rotationY度
	glScalef(g_scale_size, g_scale_size, g_scale_size);//沿x,y,z轴缩放
	glPushMatrix(); //压入当前矩阵堆栈


	//ADD TEXTURE
	//glEnable(GL_TEXTURE_2D);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);//纹理贴图与材质混合
#ifdef GL_VERSION_1_1
...
void myGlutMotion(int x, int y) //处理当鼠标键摁下时,鼠标拖动的事件,输入参数为鼠标的窗口坐标
{
